            ...
            <div>
                <div v-for="(o,index) in reduceOrder" :key="index" class="reduceOrder">
                    <div class="orderNo">
                        ...
                        <div v-if="o.status == 1" style="cursor: pointer;" @click="confirmRece(o.oid)">确认收货</div>
                        <div v-else>已完成<van-icon name="delete-o" /></div>
                    </div>
                    <div class="orderInfo">
...
//确认收货
const confirmRece = (val) =>{
    axiosInstance.post('/receive',{
        oid: val
    }).then((res) =>{
